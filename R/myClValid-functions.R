...
myClValid.cat <- function(obj, nClust, clMethods="hierarchical", validation="stability", maxitems=600,
...
these can be downloaded from Bioconductor (www.bioconductor.org)")
  ...
  for (i in 1:length(clMethods)) {
    ...
    cvalid <- my.vClusters(mat,clMethods[i],nClust, validation=validation,
  ...
  new("myClValid", clusterObjs=clusterObjs, measures=validMeasures, measNames=measures,
...
my.stability <- function(mat, Dist=NULL, del, cluster, clusterDel, method="euclidean") {
  mat <- as.matrix(one_hot(as.data.table(mat)))
  cluster <- as.matrix(one_hot(as.data.table(cluster)))
  clusterDel <- as.matrix(one_hot(as.data.table(clusterDel)))
  method <- "manhattan"
  any.na <- any(is.na(mat))
  obsNum <- 1:nrow(mat)
  nc1 <- length(table(cluster))
